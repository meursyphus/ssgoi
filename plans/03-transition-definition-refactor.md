...
- No package release, version bump, or site deployment is requested. The user
  authorized committing and pushing this work to a dedicated draft PR on `codex/transition-refactor`, split from #400.
...
5. Raw spring helpers and public API documentation.
...
This branch contains only transition APIs, preset implementation changes, docs,
and the Pinterest demo. The standalone analyzer, its agent skill, CLI scripts,
Python dependencies and analyzer plans live on `codex/motion-analyzer`.
Both branches start from `latest`; neither requires the other to merge first.
