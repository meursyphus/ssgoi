...
2. 30fps에서는 spring/double/t0 조합이 서로 구분되지 않는다(식별성 문제). 따라서
   ...
   - **duration/bounce 제약 피팅** 모드: 진단용 duration/bounce의 2-파라미터 공간에서만 피팅.
...
- 합성 타이밍: `Δ = t0(in) − t0(out)`. out 피팅 곡선에서 `Δ` 시각의 진행률을 읽으면 그게
  기존 MultiAnimation의 `startAt`이다(진행률 기준이라 적분기와 독립). 숫자 `1`은 첫 도달이며 정착과 구분한다.
...
- 코드 스니펫(복사 버튼) — 기존 core/runtime API로 물리값을 내보내고 녹화 방향을 별도로 기록:
  ```ts
  import { IntegratorProvider } from "@ssgoi/core/runtime";
  const incoming = IntegratorProvider.from({ spring: { stiffness: 410, damping: 30 } });
  const outgoing = IntegratorProvider.from({ inertia: { acceleration: 150, resistance: 1.5 } });
  ```
...
- **가장 가까운 시맨틱 프리셋**: 피팅 곡선을 `smooth/snappy/bouncy/gentle/swift/accelerate` 곡선과
  ...
  오차가 작으면 가까운 참조 프리셋 이름도 보고하되, 코드는 실제 물리값으로 내보낸다. 이 비교 결과가
