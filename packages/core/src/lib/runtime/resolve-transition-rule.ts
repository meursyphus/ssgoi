...
import { getBestPathMatch } from "../ssgoi-transition/path-pattern";
 ...
 */
export function resolveTransitionRule<TTransition>(
  from: string,
  to: string,
  rules: readonly RouteRule<TTransition>[],
  historyDirection: NavigationDirection,
): ResolvedTransitionRule<TTransition> | null {
  let best: Candidate<TTransition> | null = null;
