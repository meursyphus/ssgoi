import { releaseFillOnComplete } from "../animation-group";
...
export const sheet = (options: SheetOptions = {}) => {
  ...
  const createDirection = (navigationDirection: NavigationDirection) => {
    const direction = navigationDirection === "forward" ? "enter" : "exit";
    const sheetSide = direction === "enter" ? "to" : "from";
    const backgroundSide = direction === "enter" ? "from" : "to";
    const physics =
      direction === "enter" ? provider.enterPhysics : provider.exitPhysics;
    return {
      prepare: ({ from, to, context, createElement }) => {
        const sheet = { from, to }[sheetSide];
        const background = { from, to }[backgroundSide];
      ...
      animation: ({ from, to, context, overlay }) => {
        const sheetEl = { from, to }[sheetSide];
        const backgroundEl = { from, to }[backgroundSide];
        const sheetRect = getViewportRect(context, sheetSide);
        ...
        const sheetAnim = new WebAnimation({
          ...
          onComplete: () => {
            ...
            (
              sheetEl.style as CSSStyleDeclaration & { contain: string }
            ).contain = "";
        ...
        const anims: Record<"sheet" | "background" | "overlay", Animation> = {
          sheet: sheetAnim,
          background: new MultiAnimation([]),
          overlay: new MultiAnimation([]),
        };
        ...
        if (animatesBackground) {
          ...
          const bgRect = getViewportRect(context, backgroundSide);
          ...
          releaseFillOnComplete(bgAnim);
          anims.background = bgAnim;
        }
    ...
    } satisfies TransitionDirection<SheetExtras>;
  };
