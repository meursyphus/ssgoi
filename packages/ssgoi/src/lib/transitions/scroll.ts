...
const scroll = ({ velocity = DEFAULT_VELOCITY, delay = 0, easing, reverse }: { velocity?: number, delay?: number, easing?: (t: number) => number, reverse?: boolean } = {}) => ({
	in(node: Element) {
...
export const scrollUpToDown: Transition = ({ velocity, delay, easing }: { velocity?: number, delay?: number, easing?: (t: number) => number } = {}) => ({
	in(node) {
...
export const scrollDownToUp: Transition = ({ velocity, delay, easing }: { velocity?: number, delay?: number, easing?: (t: number) => number } = {}) => ({
	in(node) {
