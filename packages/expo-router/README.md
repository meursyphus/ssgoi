...
This is the experimental first milestone in [#402](https://github.com/meursyphus/ssgoi/issues/402), published in lockstep with the web packages. A stable package version does not imply production-ready native support. Start with [`templates/expo`](https://github.com/meursyphus/ssgoi/tree/HEAD/templates/expo) from the matching release for the supported dependency baseline and device checks.

## Expo Router

The example pins Expo SDK 56.0.21, Expo Router 56.2.20, React Native 0.85.3, React 19.2.3, Reanimated 4.3.1, and Worklets 0.8.3. Expo's standard navigator API is alpha; newer SDKs need separate validation. The Expo Router peer is pinned to that validated version because the integration API is unstable. The underlying `@ssgoi/react-native` package does not depend on Expo Router.
...
pnpm --filter @ssgoi/expo-router test:run
SSGOI_TEST_PLATFORM=android pnpm --filter @ssgoi/expo-router test:run
