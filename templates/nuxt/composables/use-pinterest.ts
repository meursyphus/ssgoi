...
const pinContents: Record<string, any> = {
  Design: {
...
export function usePinterestItem(id: string) {
  return pinterestItems.find((item) => item.id === id);
}

export function useAllPinterestItems() {
  return pinterestItems;
}

export function useRelatedPins(currentId: string, limit: number = 6) {
  const currentPin = usePinterestItem(currentId);
