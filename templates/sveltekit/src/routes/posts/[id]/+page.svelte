<script lang="ts">
  ...
  function renderMarkdown(content: string) {
    return content
      .replace(
        /^# (.*$)/gim,
        '<h1 class="text-lg font-medium text-white mb-4 mt-6">$1</h1>',
      )
      .replace(
        /^## (.*$)/gim,
        '<h2 class="text-base font-medium text-white mb-3 mt-5">$1</h2>',
      )
      .replace(
        /^### (.*$)/gim,
        '<h3 class="text-sm font-medium text-white mb-2 mt-4">$1</h3>',
      )
      .replace(
        /\*\*(.*?)\*\*/g,
      ...
      .replace(
        /\n\n/g,
        '</p><p class="text-xs text-neutral-300 mb-4 leading-relaxed">',
      )
      .replace(
        /^(?!<[hl])/gim,
