...
- `composeMeasuredTracks` is included only when predecessor progress thresholds represent the measured ordering within one 60 Hz frame. Numeric thresholds, including 1, use first crossing rather than settling. Delays beyond settling and non-monotonic first-crossing conflicts return `startAt: null`. A custom transition can explicitly use `set({ startAt: { after, at: "settled" } })` when it needs a completion dependency. Do not replace missing schedule values with arbitrary thresholds. A sample with sparse early visibility may have timing bias even when its curve RMSE is small.
- `--preset NAME` records a requested preset hint. Selecting a matching preset and its geometry remains Codex's job; use the preset's named groups and preserve shared progress where its geometry requires it.
