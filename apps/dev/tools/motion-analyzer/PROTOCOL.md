...
This tool works on `latest` without the transition-refactor PR. Exported recipes
use `WebAnimation` / `MultiAnimation` from `@ssgoi/core` and
`IntegratorProvider.from({ spring: ... })` or `IntegratorProvider.from({ inertia: ... })`
from the existing `@ssgoi/core/runtime` entry. The viewer's copy button uses the
same API. No new `spring()` helper, named-child API, or preset override is required.
The named semantic curves in the report are analyzer reference curves, not imports
that the consuming core version must export.
...
- `--preset NAME` records a requested preset hint. Selecting a matching preset and its geometry remains Codex's job; use a custom animation when per-element fits differ from the preset's existing choreography.
