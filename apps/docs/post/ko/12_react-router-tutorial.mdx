---
title: "React Router로 네이티브 앱 같은 웹앱 만들기"
description: "React Router와 SSGOI를 사용해 네이티브 앱 같은 페이지 전환 효과를 구현하는 방법을 단계별로 알아봅니다."
...
오늘은 React Router로 **네이티브 앱 같은 페이지 트랜지션이 들어간 웹앱**을 만들어봅니다.
...
`app/components/posts/index.tsx`를 보면:
...
`app/components/posts/detail.tsx`를 보면:
...
`products_.layout.tsx`에서는 중첩된 `Ssgoi`를 사용해 탭 영역만 트랜지션합니다:
...
- **React Router 템플릿**: https://github.com/meursyphus/ssgoi/tree/main/templates/react-router
