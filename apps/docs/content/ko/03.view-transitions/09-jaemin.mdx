...
jaemin({
  ...
  containerMode?: 'auto' | 'positioned-parent' | 'viewport'; // 컨테이너 감지 모드 (기본값: 'auto')
})
```

### 컨테이너 모드

`containerMode` 옵션은 전환이 컨테이너 경계를 계산하는 방법을 제어합니다:

- **`'auto'`** (기본값): 환경을 자동으로 감지합니다. positioned parent가 뷰포트의 80%보다 작으면 그 경계를 사용하고, 그렇지 않으면 전체 뷰포트를 사용합니다
- **`'positioned-parent'`**: 항상 가장 가까운 positioned parent를 컨테이너 경계로 사용합니다. 데모, 모달, 제한된 레이아웃에 이상적입니다
- **`'viewport'`**: 항상 전체 브라우저 뷰포트를 컨테이너 경계로 사용합니다. 전체 페이지 애플리케이션에 최적입니다

```typescript
// 자동 감지 (대부분의 경우 권장)
jaemin()

// 데모/모달을 위한 명시적 컨테이너 제어
