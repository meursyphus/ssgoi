---
title: "SSGOI 소개"
description: "SSGOI는 웹에서 네이티브 앱과 같은 자연스러운 페이지 전환 애니메이션을 구현할 수 있게 해주는 라이브러리입니다."
nav-title: "소개"
---

<Note type="tip">SSGOI는 "쓰고이"라고 읽으며, 하여튼 좋다는 뜻입니다!</Note>

<Demo autoPlay={false} />

## 왜 SSGOI인가?

### 1. 모든 브라우저에서 동작

브라우저 크롬 View Transition API와 달리, SSGOI는 모든 모던 브라우저에서 일관된 경험을 제공합니다. Chrome, Firefox, Safari 어디서든 동일한 애니메이션을 보장합니다.

<Grid cols={3}>
  <Card title="Chrome">
    <Badge variant="success">✓ 완벽 지원</Badge>
  </Card>
  <Card title="Firefox">
    <Badge variant="success">✓ 완벽 지원</Badge>
  </Card>
  <Card title="Safari">
    <Badge variant="success">✓ 완벽 지원</Badge>
  </Card>
</Grid>

### 2. 서버사이드 렌더링 완벽 지원

- **SSR/SSG 호환**: Next.js, Nuxt, SvelteKit 등에서 하이드레이션 문제 없이 작동
- **SEO 친화적**: 애니메이션이 검색 엔진 최적화에 영향을 주지 않음
- **초기 로딩 최적화**: 서버에서 렌더링된 HTML을 그대로 활용

### 3. 프레임워크 라우팅 그대로 사용

<Note>
  SSGOI는 기존 라우팅 시스템과 완벽하게 호환됩니다. 라우터를 변경할 필요가
  없습니다!
</Note>

기존 프레임워크의 라우팅을 그대로 사용하면서 애니메이션만 추가할 수 있습니다. Next.js의 `Link`, SvelteKit의 `goto`, Vue Router 등 모든 라우팅 방식과 호환됩니다.

## 주요 특징

<CodeDemo>
```typescript
// 단 몇 줄의 코드로 시작
const config = {
  defaultTransition: fade(),
  transitions: [
    {
      from: '/home',
      to: '/about',
      transition: scroll({ direction: 'up' })
    }
  ]
}
```
</CodeDemo>

### 🎨 다양한 전환 효과

- **Fade**: 부드러운 페이드 인/아웃
- **Slide**: 방향성 있는 슬라이드
- **Scale**: 확대/축소 효과
- **Hero**: 공유 요소 애니메이션
- **Pinterest**: Pinterest 스타일 확장
- **Ripple**: Material Design 리플 효과

### ⚡ 극한의 성능 최적화

SSGOI는 **Web Animation API**와 **스프링 물리 엔진**을 결합하여 최상의 성능을 제공합니다.

- **Web Animation API 기반**: 스프링 물리 연산을 미리 계산하여 키프레임으로 변환, 메인 스레드 부하 최소화
- **GPU 가속**: transform, opacity 등 합성 레이어 속성을 활용한 60fps 애니메이션
- **애니메이션 상태 기억**: 브라우저 뒤로가기/앞으로가기 시에도 자연스러운 전환
- **자연스러운 물리 기반 움직임**: 스프링 애니메이션으로 기계적이지 않은 자연스러운 모션
- **작은 번들 사이즈**: < 20KB gzipped, Tree-shaking 지원

### 🛠️ 개발자 친화적

- TypeScript 완벽 지원
- 직관적인 API 설계
- 풍부한 문서와 예제
- React, Svelte, Vue, Angular 등 모든 주요 프레임워크 지원

## 시작하기

<Note type="info">
  SSGOI는 React, Svelte, Vue, Angular를 공식 지원합니다.
</Note>

다음 섹션에서 빠른 시작 가이드를 통해 SSGOI를 프로젝트에 적용하는 방법을 알아보세요!
