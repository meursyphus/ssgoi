...
interface DrillOptions {
  ...
  physics?: {
    inertia?: {
      acceleration?: number;    // Inertia acceleration (default: 20)
      resistance?: number;      // Resistance coefficient (default: 1.5)
    };
  };
}
```
        ...
        transitions: [
          {
            ...
            transition: drill({ 
              opacity: true,
              physics: { inertia: { acceleration: 25, resistance: 1.2 } }
            })
          }
        ]
      };
...
drill({
  opacity: true,
  physics: { inertia: { acceleration: 15, resistance: 1.8 } }
})
```
...
// Smooth drill
drill({
  physics: { inertia: { acceleration: 15, resistance: 2.0 } }
})
```
