...
```
api/{domain}/
  ├── index.ts          # typed local API (resolveActions로 묶어서 export)
  ...
  │     └── index.ts
  └── actions/
        └── *.ts        # find / findAll / create / ... 각 local mock action
```

- `data.ts` / `data/`는 **이 도메인 폴더 안에서만** import한다. 다른 도메인/state/page에서 직접 참조 금지 — 반드시 api(local mock action)를 거친다.
...
- **Local mock actions**: 모든 메서드는 async 함수. `'use server'`를 붙이지 않는다. 데모 mock은 클라이언트 state query에서도 바로 실행되어야 하며, 서버 액션 네트워크 왕복을 만들지 않는다.
- **Frontend-friendly**: UI에서 바로 쓸 수 있게 변환해서 반환
  ...
  ```
- **Latency 시뮬레이션 금지**: actions 내부에 `sleep`, `setTimeout`, artificial delay를 넣지 않는다. 데모는 ssgoi 트랜지션이 주역이라 mock API 지연이 있으면 전환이 답답해 보인다.
...
3. `actions/*.ts` — local mock action 구현 (data 읽고 변환)
4. `index.ts` — typed local API
...
## actions/*.ts (Local Mock Actions)

- `'use server'` 금지
- 모든 export는 `createAction`으로 감싼다 (에러 메시지 노출 처리 + 호출 형태 통일)
...
- `createAction`은 `ActionError`만 safe result로 변환한다
