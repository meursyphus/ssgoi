// 기본 사용법
export const fadeBasicReact = `import { transition } from '@ssgoi/react';
...
export const fadeBasicSvelte = `<script>
...
// 부분 페이드
export const partialFadeReact = `const partialFade = fade({
...
export const partialFadeSvelte = `<script>
  import { transition } from '@ssgoi/svelte';
  import { fade } from '@ssgoi/svelte/transitions';
...
// 느린 페이드
export const slowFadeReact = `const slowFade = fade({
  spring: { 
    stiffness: 100,  // 낮은 강도
    damping: 20      // 낮은 감쇠
  }
...
export const slowFadeSvelte = `<script>
  import { transition } from '@ssgoi/svelte';
  import { fade } from '@ssgoi/svelte/transitions';
...
// 빠른 페이드
export const fastFadeReact = `const fastFade = fade({
  spring: { 
    stiffness: 500,  // 높은 강도
    damping: 40      // 높은 감쇠
  }
...
export const fastFadeSvelte = `<script>
  import { transition } from '@ssgoi/svelte';
  import { fade } from '@ssgoi/svelte/transitions';
...
// 조합 애니메이션
export const fadeAndScaleReact = `// 커스텀 조합 애니메이션
const fadeAndScale = {
  in: (element) => ({
    spring: { stiffness: 300, damping: 30 },
    tick: (progress) => {
...
export const fadeAndScaleSvelte = `<script>
  // 커스텀 조합 애니메이션
  const fadeAndScale = {
    in: (element) => ({
      spring: { stiffness: 300, damping: 30 },
...
// 접근성
export const accessibleFadeReact = `<div 
...
export const accessibleFadeSvelte = `<div 
