  ...
  <SsgoiTransition id="/">
    <div class="app-container">
      <div class="hero-section">
        <h1 class="app-title">SSGOI Vue Demo</h1>
      </div>
      ...
      <div class="toggle-section">
        <button @click="showShapes = !showShapes" class="toggle-button">
...
import {
  SsgoiTransition,
  vTransition as vTransitionDirective,
} from "@ssgoi/vue";
...
const damping = ref(30);

function setSpeed(newStiffness: number, newDamping: number) {
  stiffness.value = newStiffness;
  damping.value = newDamping;
}

// Transition creation functions - these return the actual transition config
function fadeTransition(element: HTMLElement) {
  return {
...
function scaleRotateTransition(element: HTMLElement) {
  return {
    spring: { stiffness: stiffness.value, damping: damping.value },
    tick: (progress: number) => {
...
function slideTransition(element: HTMLElement) {
  return {
    spring: { stiffness: stiffness.value, damping: damping.value },
    tick: (progress: number) => {
...
function bounceScaleTransitionIn(element: HTMLElement) {
  return {
    spring: {
...
function bounceScaleTransitionOut(element: HTMLElement) {
  return {
    spring: { stiffness: stiffness.value, damping: damping.value },
    tick: (progress: number) => {
      const scale = 0.5 + progress * 0.5;
